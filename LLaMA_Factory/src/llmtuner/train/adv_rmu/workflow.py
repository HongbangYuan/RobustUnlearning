...
from .trainer import AdvRMUTrainer
...
def run_adv_rmu(
    ...
    print("Adv rmu perturb layer:",finetuning_args.adv_rmu_perturb_layer)
    ...
    if is_multi_prompt(dataset):
        ...
        if is_tofu(data_args):
            data_collator = MultiPromptDataCollatorForSeq2Seq(
                tokenizer=tokenizer,
            )
        else:
    ...
    if data_args.dataset_name == 'rwku':
        ...
        output_result_dir = data_args.output_result_dir
        print(f"Output result dir:{output_result_dir}")
