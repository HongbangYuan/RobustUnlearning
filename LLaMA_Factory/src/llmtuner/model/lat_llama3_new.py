...
class LATLlamaAttention(LlamaAttention):
    def forward(
        ...
        **kwargs,
        ...
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states.to(query_states.device), cos.to(query_states.device), sin.to(query_states.device))
        ...
        attn_output = torch.matmul(attn_weights, value_states.to(attn_weights.device))
        ...
        if self.config.pretraining_tp > 1:
            ...
            attn_output = self.o_proj(attn_output.to(self.o_proj.weight.device))
...
class LATLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        # # print("Before the attention computing:")
        # # print(f"self._use_flash_attention_2: {self._use_flash_attention_2}")
        # # print("self._use_sdpa:",self._use_sdpa)
