# src: https://github.com/huggingface/transformers/blob/main/src/transformers/models/mistral/modeling_mistral.py
...
class LATMistralMLP(MistralMLP):
...
class LATMistralAttention(MistralAttention):
    """
...
# TODO(joao): add me back asap :)
class LATMistralDecoderLayer(MistralDecoderLayer):

    def __init__(self, config: MistralConfig, layer_idx: int):
        super().__init__(config,layer_idx)
        self.hidden_size = config.hidden_size

        self.self_attn = LATMistralAttention(config=config, layer_idx=layer_idx)
        self.mlp = LATMistralMLP(config)
...
class LATMistralModel(MistralModel):
    """
    ...
    def __init__(self, config: MistralConfig):
        ...
        self.layers = nn.ModuleList(
            [LATMistralDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LATMistralForCausalLM(MistralForCausalLM):

    def __init__(self, config):
        super().__init__(config)
        self.model = LATMistralModel(config)
        self.vocab_size = config.vocab_size
