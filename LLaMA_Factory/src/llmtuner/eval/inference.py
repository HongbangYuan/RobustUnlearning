...
@torch.no_grad()
def cal_normalized_conditional_probability(model, tokenizer, scoring_examples, batch_size=1, aggregation="sum",
                                           disable_tqdm=False):
    '''
    ...
    for i in range(0, len(unrolled_examples), batch_size):
        ...
        for example_idx, (prompt, example) in enumerate(zip(batch_prompts, batch_examples)):
            ...
            completion_logits = example_logits[len(tokenized_prompt):len(tokenized_example), :]
            labels = completion_ids.clone().to(completion_logits.device)
            gt_loss = get_batch_loss(completion_logits.unsqueeze(0), labels.unsqueeze(0))
            num_tokens = completion_logits.shape[0]
            avg_gt_loss = (gt_loss / num_tokens).item()

            score = {
                "gt_loss": gt_loss.item(),
                "avg_gt_loss": avg_gt_loss,
                "num_tokens_gt": num_tokens,
            }
            scores.append(score)
